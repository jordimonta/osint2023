...
function maltego() {
	...
	cd $githome/maltego
	wget https://maltego-downloads.s3.us-east-2.amazonaws.com/linux/Maltego.v4.3.1.deb >/dev/null 2>&1
	check
...
function osrframework() {
	...
	ln -s /home/manager/.local/bin/osrf $userdesktop/General/osrf >/dev/null 2>&1
}
...
function instaloader() {
	# instaloader
	echo -ne "\n${yellow}[*]${endC}${blue} Instalación ${end}${purple}instaloader ${end}${blue}...${end}"
	pip3 install instaloader >/dev/null 2>&1
	checkV

	echo -e "\n\t${yellow}Ejecución:${end}\n"
	echo -e "\t${yellow}\t>sudo osrf --help ${end}\n"

	ln -s /home/manager/.local/bin/instaloader $userdesktop/Redes_Sociales/instaloader >/dev/null 2>&1
}

function iarachnidspider() {
	# instaloader
	echo -ne "\n${yellow}[*]${endC}${blue} Instalación ${end}${purple}arachnid-spider ${end}${blue}...${end}"
	pip install arachnid-spider >/dev/null 2>&1
	checkV

	echo -e "\n\t${yellow}Ejecución:${end}\n"
	echo -e "\t${yellow}\t>sudo arachnid -h ${end}\n"

	ln -s /home/manager/.local/bin/arachnid $userdesktop/General/arachnid >/dev/null 2>&1
}

function mediainfo() {
	# exiftool
	echo -ne "\n${yellow}[*]${endC}${blue} Instalación ${end}${purple}mediainfo-gui${end}${blue}...${end}"
	instalator mediainfo-gui
	checkV

	echo -e "\n\t${yellow}Ejecución:${end}\n"
	echo -e "\t${yellow}\t>mediainfo-gui${end}\n"

	ln -s /usr/bin/mediainfo-gui $userdesktop/Metadatos/mediainfo-gui >/dev/null 2>&1
}
